...
class AppDeployIntegrationTest extends AkkaIntegrationTest with EmbeddedMarathonTest {
  ...
  "AppDeploy" should {
    ...
    "increase the app count metric when an app is created" in {
      ...
      val appCount = (marathon.metrics().entityJson \ "gauges" \ "marathon.apps" \ "mean").as[Double]
      ...
      result should be(Created)
      eventually {
        (marathon.metrics().entityJson \ "gauges" \ "marathon.apps" \ "max").as[Double] should be > appCount
      }
    }
